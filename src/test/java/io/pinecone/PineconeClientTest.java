...
public class PineconeClientTest {
    ...
    public void PineconeWithOkHttpClientAndUserAgent() throws IOException {
        ...
        assertEquals("lang=java; pineconeClientVersion=v0.8.0", requestCaptor.getValue().header("User-Agent"));
    }
    ...
    public void PineconeWithSourceTag()  throws IOException {
        ...
        assertEquals(expectedIndex, index);
        verify(mockClient, times(1)).newCall(requestCaptor.capture());
        assertEquals("lang=java; pineconeClientVersion=v0.8.0; source_tag=testsourcetag", requestCaptor.getValue().header("User-Agent"));
    }
}